...
class SearchEntryQuickPickItem implements vscode.QuickPickItem {
	...
	constructor(result: SearchResult) {
		this.result = result;
		this.label = result.filePath;
		this.description = result.lineNumber.toString();
		this.detail = result.line;
	...
	let disposable = vscode.commands.registerCommand('fuzz.search', () => {
		...
		qp.onDidChangeValue(async e => {
			try {
				...
				qp.items = results.slice(0,20).map((r) => new SearchEntryQuickPickItem(r));
			} catch (e) {
				if (typeof e === "string") {
